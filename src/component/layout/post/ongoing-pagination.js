...
const OngoingPagination  = ({pagination,value}) => {
  const active = 'cursor-text font-bold bg-blue-500 text-white border border-gray-300    dark:border-gray-700 dark:bg-blue-500 dark:text-white'
    ...
    const nextUrl = `${value.path}//page/${parseInt(value?.page)+1}`
    const prevUrl = `${value.path}/page/${parseInt(value?.page)-1}`
 console.log(value);
    ...
    return(
        ...
        <ul class="inline-flex -space-x-px text-xs sm:text-sm flex-wrap">
          <li>
           ...
           {
              pagination?.pageNumber?.slice(1, pagination?.pageNumber?.length).map(page => {
                ...
                {  value.page === page  ? (
                  ...
                  : (
                      <Link  href={`${value.path}/page/${parseInt(page)}`} className={`flex items-center justify-center h-8 px-4 sm:h-10 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 ${normal}`}>{page}</Link>
                  )
              }
            </li>
              })
