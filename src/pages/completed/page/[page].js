...
const value = {
  ...
  type:'ongoing'
    ...
    return(
<Fragment>
<MetaHead  title={`Anime Complete Page ${query.page} - ${siteInfo?.menus?.globalSeo?.siteName}`} description={`Halaman anime complete, cari anime untuk di tonton maupun download hanya di - ${siteInfo?.menus?.globalSeo?.siteName}`} />
...
export async function getServerSideProps(context) {
  ...
  try {
    const response = await axios.get(`${path}/api/v1/complete?page=${page}`, {
      next: {
        revalidate: 10, 
        cache: 'force-cache',
      },
